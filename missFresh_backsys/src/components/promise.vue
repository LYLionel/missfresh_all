		...
		methods: {
			...
			search(msg) {
				axios({
					...
					this.dataArray = res.data.products_list;
					for(var i in this.dataArray) {
						this.dataArray[i].id = parseInt(i) + 1;
						this.dataArray[i].icon = ('/static/upload/' + res.data.products_list[i].icon);
					}
					this.total = this.dataArray.length;
					this.tableData = this.dataArray.slice(0, this.pageSize)
					this.currentPage3 = 1;
			...
			init(){
				...
				method:'get'
					...
					this.dataArray = result.data.products_list;
					for(var i in this.dataArray){
						this.dataArray[i].id = parseInt(i) + 1;
						this.dataArray[i].icon = ('/static/upload/' + result.data.products_list[i].icon);
					}
					this.total = this.dataArray.length;
					this.tableData = this.dataArray.slice(0,this.pageSize);
				})
